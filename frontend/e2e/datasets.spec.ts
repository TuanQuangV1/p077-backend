...
test("datasets table lists rosbag captures from the backend", async ({ page }) => {
    ...
    // Registry now shows analysisStatus (diagnosis lifecycle) instead of raw upload status
    const displayStatus = (payload.items[0] as any).analysisStatus ?? payload.items[0].status
    await expect(page.getByText(displayStatus).first()).toBeVisible()
})
...
test("filter narrows the dataset table", async ({ page }) => {
    ...
    await page.getByPlaceholder("Lọc theo tên tệp...").fill(names[0])
...
test("datasets expose upload and delete actions", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /Tải rosbag lên/ })).toBeVisible()
    await expect(page.getByRole("button", { name: /Phân tích mục đã chọn/ })).toBeVisible()
