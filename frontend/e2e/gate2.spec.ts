...
async function uploadAndAnalyze(page: import("@playwright/test").Page, file: string, name: string) {
  ...
  const analyzeButton = row.getByRole("button", { name: "Phân tích" })
...
test("Gate 2 healthy upload renders a green result without detections", async ({ page }) => {
  ...
  await expect(page.getByText("0 phát hiện").first()).toBeVisible()
  await expect(page.getByText("KHOẺ MẠNH").first()).toBeVisible()
})
...
test("Gate 2 anomaly upload renders detection and labelled LLM fallback", async ({ page }) => {
  ...
  await expect(page.getByText("Có phân tích LLM").first()).toBeVisible()
  await expect(page.getByText("canned-fallback").first()).toBeVisible({ timeout: 30_000 })
  await expect(page.getByText(/Điểm sức khỏe.*vấn đề nghiêm trọng\/cao/)).toBeVisible()
})
...
test("Gate 2 invalid upload shows the backend validation error and stays usable", async ({ page }) => {
  ...
  await expect(page.getByText("Tải lên thất bại: unsupported file type: .txt")).toBeVisible()
  await expect(page.getByRole("heading", { name: "Tập dữ liệu" })).toBeVisible()
  await expect(page.getByRole("button", { name: /Tải rosbag lên/ })).toBeEnabled()
})
