...
import type { DemoAnomalyKind } from "@/lib/types"
...
export async function GET() {
  ...
  // d.anomalies is generated by the mock store, which only ever produces
  // DemoAnomalyKind values (see lib/server/store.ts).
  const counts = new Map<DemoAnomalyKind, number>()
  for (const a of d.anomalies) {
    const kind = a.kind as DemoAnomalyKind
