...
function Dashboard({ overview, navigate }: { overview: Overview | null; navigate: (href: string) => void }) { const t = overview?.totals ?? {}; return <><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><StatTile label="Tệp Rosbag đã phân tích" value={t.analyzed ?? "--"} hint={`${t.rosbags ?? 0} đã đăng ký · ${t.hoursOfData ?? 0}h dữ liệu`} icon={<DatabaseIcon className="size-4" />} /><StatTile label="Tỷ lệ lượt chạy có lỗi" value={t.runsWithIssuesPct ? `${t.runsWithIssuesPct}%` : "--"} tone="critical" hint={`${t.anomalies ?? 0} bất thường phát hiện`} icon={<CircleAlertIcon className="size-4" />} /><StatTile label="Thời gian chẩn đoán TB" value={t.meanTimeToDiagnoseSec ?? "--"} unit="giây" hint="từ phân tích đến kết luận AI" icon={<ActivityIcon className="size-4" />} /><StatTile label="Chi phí suy luận AI" value={t.inferenceCostUsd ? `$${t.inferenceCostUsd}` : "--"} hint={`${compact(t.tokens ?? 0)} tokens tiêu thụ`} icon={<CpuIcon className="size-4" />} /></div><div className="grid gap-4 xl:grid-cols-[1.5fr_1fr]"><SectionCard title="Lượt phân tích gần đây" description="Các tác vụ phân tích rosbag mới nhất" actions={<Button variant="ghost" size="sm" onClick={() => navigate("/analysis")}>Mở Không gian làm việc <ArrowRightIcon data-icon="inline-end" /></Button>}><div className="divide-y divide-border">{overview?.recentRuns.map((run) => <button key={run.id} onClick={() => navigate("/analysis")} className="flex w-full items-center gap-3 py-3 text-left hover:bg-accent/30"><StatusLabel status={run.status} /><span className="min-w-0 flex-1 truncate text-sm">{run.rosbagName}</span><span className="font-mono text-xs text-muted-foreground">{run.anomalyCount} lỗi</span><span className="hidden font-mono text-xs text-muted-foreground sm:block">{ms(run.totalLatencyMs)}</span></button>) ?? <p className="text-sm text-muted-foreground">Đang tải dữ liệu...</p>}</div></SectionCard><SectionCard title="Phân loại sự cố" description="Các dạng lỗi bất thường được phát hiện"><div className="flex flex-col gap-3">{overview?.topIssues.slice(0, 5).map((i) => <div key={i.label} className="flex items-center gap-3"><span className="w-40 truncate text-xs text-muted-foreground">{i.label}</span><div className="h-2 flex-1 rounded-full bg-muted"><div className="h-full rounded-full bg-primary" style={{ width: `${Math.min(100, i.count * 16)}%` }} /></div><span className="w-6 text-right font-mono text-xs">{i.count}</span></div>)}</div></SectionCard></div></> }

function Analysis({ activeRun, anomalies, selected, setSelected, lanes, playhead, setPlayhead, selectedResult }: { activeRun: AnalysisRun | null; anomalies: Anomaly[]; selected: string | null; setSelected: (s: string) => void; lanes: Lane[]; playhead: number; setPlayhead: (n: number) => void; selectedResult?: AIResult }) { return <div className="grid min-h-[680px] gap-4 xl:grid-cols-[230px_minmax(0,1fr)_380px]"><Card className="min-h-0 overflow-hidden"><CardHeader className="border-b border-border py-3"><CardTitle className="text-sm">Danh sách Bất thường</CardTitle></CardHeader><AnomalyList anomalies={anomalies} selectedId={selected} severities={[]} onSeveritiesChange={() => { }} onSelect={(a) => { setSelected(a.id); setPlayhead(a.tSec) }} /></Card><Card className="min-w-0 overflow-hidden"><CardHeader className="flex-row items-center justify-between border-b border-border py-3"><div><CardTitle className="text-sm">Dòng thời gian tin nhắn (Timeline)</CardTitle><p className="font-mono text-[10px] text-muted-foreground">{activeRun?.rosbagName ?? "Đang tải lượt chạy"} · kéo để cuộn</p></div><Badge variant="outline" className="font-mono text-[10px]">{clock(playhead)}</Badge></CardHeader><CardContent className="p-0 pt-3"><TimelineCanvas durationSec={120} lanes={lanes} anomalies={anomalies} playhead={playhead} view={{ from: 0, to: 120 }} selectedAnomalyId={selected} onScrub={setPlayhead} onViewChange={() => { }} onSelectAnomaly={setSelected} /></CardContent></Card><div className="min-h-0 overflow-auto">{selectedResult ? <AIConclusion result={selectedResult} anomaly={anomalies.find((a) => a.id === selectedResult.anomalyId)} onSeek={setPlayhead} /> : <Card><CardContent className="p-5 text-sm text-muted-foreground">Chọn một phát hiện bất thường để xem kết luận từ AI.</CardContent></Card>}</div></div> }
...
function DatasetRegistry({ bags, onRefresh, navigate }: { bags: Rosbag[]; onRefresh: () => void; navigate: (href: string) => void }) {
    ...
    const analyze = async (bag: Rosbag) => { setBusy(bag.id); try { const result = await post<{ run: AnalysisRun }>("/api/runs", { rosbag_id: bag.id }); toast.success("Analysis completed", { description: result.run.id }); window.location.assign("/analysis") } catch { toast.error("Unable to run analysis") } finally { setBusy(null) } }
